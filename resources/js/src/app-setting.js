...
export default {
    ...
    toggleLanguage(item) {
        ...
        if (item) {
            ...
            let code = store.state.setting.locale;
            if (!code) {
                code = localStorage.getItem("i18n_locale");
            }

            item = store.state.setting.countryList.find((d) => d.code === code);
            if (item) {
                lang = item;
            }
        }
        ...
        if (!lang) {
            lang = store.state.setting.countryList.find((d) => d.code === "en");
        }

        store.commit("setting/toggleLocale", lang.code);
        return lang;
    },
    ...
    toggleMode(mode) {
        ...
        store.commit("setting/toggleDarkMode", mode || "light");
        return mode;
    },
};
